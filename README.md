...
后端调用阿里云百炼 Qwen-VL（密钥藏在服务端），前端（Vite）负责拍照、压缩、展示。

## 两种使用方式

- **线上访问**：项目已部署到 Vercel，手机浏览器直接打开域名即可用，无需安装任何东西。见 [一、线上访问（Vercel）](#一线上访问vercel)。
- **本地运行**：在自己电脑上跑前后端，用于开发调试。见 [二、本地运行（开发）](#二本地运行开发)。

两套共用同一份业务逻辑，互不干扰：
- 本地开发入口：`server/src/index.ts`（Express 长驻）
- Vercel 函数入口：`api/scan.ts`（自包含 Serverless 函数）
...
手机浏览器打开生产域名（形如 `项目名.vercel.app`）即可，看到拍照界面就能扫。域名在 Vercel 后台 Settings → Domains 查看，固定不变。

### 自己部署

1. 在 [Vercel](https://vercel.com) 导入这个 Git 仓库
2. Framework 选 **Other**，Build Command 保持 `npm run build:web`，Output Directory 保持 `web/dist`
3. Settings → Environment Variables 添加（环境全选 **Production**）：
   - `QWEN_API_KEY` — 阿里云百炼 API Key
   - `QWEN_BASE_URL` — `https://dashscope.aliyuncs.com/compatible-mode/v1`
   - `QWEN_MODEL` — `qwen-vl-plus`
4. Deploy，完成后去 Settings → Deployment Protection → **关闭 Vercel Authentication**（否则访问会要求登录 Vercel 账号）
5. 之后每次 `git push` 到 GitHub，Vercel 会自动重新部署

> 说明：`api/scan.ts` 是自包含的 Serverless 函数，所有业务逻辑内联在单文件中，不依赖 `server/src/`。前端线上请求同域名下的 `/api/scan`，无需改接口地址。
...
电脑浏览器打开 `http://localhost:5173`，看到拍照界面即可使用。

### 4. 用手机访问本地服务

想在手机上测本地版本（线上版直接打开 Vercel 域名即可，不用这步）：
...
### 本地启动相关
- **后端报 key 错误** → `server/.env` 的 base_url 多写了 `/v1`，或没填真实 key。
...
- **"AI 走神了"** → 可能原因：
  ...
  - API key 余额不足或失效
  - 网络问题导致请求超时
  - 模型返回格式不符合预期（查看日志 `[vision] extractJson failed`）
- **扫描超时（60 秒）** → 图片过大或网络慢，前端会自动压缩到 1.8MB，但首次上传可能较慢；或检查 `QWEN_BASE_URL` 是否正确。
- **识别结果不准确** → 模型对比经验：
